...
class Autogen_variable_data_gen():
    ...
    def WBS_ID(self,column_name, parameter_value, Data_dependency, json_ui, concat_dataframe):
        ...
        if Data_dependency.lower()=='N'.lower():
            ...
            result_dataframe_temp = pd.DataFrame([])
            extracted_json = json.loads(json_ui)
            ...
            try:
                ...
                result_dataframe_temp[column_name] = string_list
                if int(number_of_l2_records) == int(1):
                    result_dataframe = result_dataframe_temp
                else :
                    result_dataframe = result_dataframe_temp.loc[
                        sorted([*result_dataframe_temp.index] * iterations * int(number_of_l2_records))].reset_index(
                        drop=True)
